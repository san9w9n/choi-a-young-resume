...
| `npm run check:copy`   | 이력서 문안 길이 검사 (문장 45자·본문 1370자, 후기 인용 제외) |
...
--tint:#f4f3f1;      /* 옅은 채움 — 후기 섹션 배경 · 사진칸 */
...
이력서는 읽는 문서가 아니라 훑는 문서라, `npm run check:copy` 가 길이를 잠급니다.
...
| 본문 전체                         | 1370자      |
| 후기 본문 (`voices.items[].body`) | 없음        |

후기 본문에만 상한이 없습니다. 보호자가 직접 쓴 말을 옮긴 것이라 우리가 문장을
자를 수 없고, 자르면 인용이 아니게 됩니다. 카드가 내용에 맞춰 늘어나며, 검사는
상한 대신 인용 총량을 따로 보고합니다. 여러 문단이면 `body` 에 배열을 넣으세요.
후기를 소개하는 `quote` 한 줄은 우리가 고른 문장이라 45자 규칙을 그대로 받습니다.
...
- **프로필 사진** — `app/components/Hero.tsx` 의 `<div className="slot portrait">` 안에 `<img>` 를 넣으면 칸을 꽉 채웁니다.
  ...
  ```
- **인쇄 / PDF** — 브라우저 인쇄를 하면 내비게이션과 사진칸이 빠지고 흑백으로 정리된 이력서 형태로 출력됩니다.
